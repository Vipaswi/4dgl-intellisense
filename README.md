...
names the suggestion (and offers it as a Quick Fix). A name that resembles nothing recognised is left alone on purpose — the
...
Where a problem has one obvious mechanical repair, `Ctrl+.` (`Cmd+.`) on the squiggle applies it:

| Problem | Offered |
|---|---|
| `gfx_Lyne(...)`, `BLEU` | **Change to `gfx_Line`** / **`BLUE`**, or **add the name to `knownNames`** if it's real but undocumented |
| `x = 1;` | **Change to `:=`** (assign) or **`==`** (compare) |
| `while (a) … endif` | **Change to `wend`** — and for `repeat`, both `until` and `forever` |
| `if (a) {` | **Remove `{`** |

Nothing is offered where nothing can be known: an unclosed block gets no fix, because
there's no way to tell where the missing terminator belongs, and a confidently wrong
edit is worse than no lightbulb.
